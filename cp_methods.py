"""
cp_methods.py  —  Conformal Prediction methods.

Paper's methods (using SPCI_and_EnbPI class):
  EnbPI: B=20, use_SPCI=False, past_window=300, stride=n_horizons
  SPCI:  B=10, use_SPCI=True,  past_window=300, stride=n_horizons, quantile_regr=True

Additional methods:
  SCP:    Split Conformal Prediction
  QRA-R:  Quantile Regression Averaging (Residuals)
  QRA-CP: Quantile Regression Averaging (Conformal Prediction)

Base models for EnbPI/SPCI (single output, paper exact):
  KNN : KNeighborsQuantileRegressor(q=[0.50], n_neighbors=290, p=1, leaf_size=1)
  LEAR: Lasso(max_iter=2500, alpha=lear_alpha)
  RF  : RandomForestRegressor(max_depth=70/80, max_features=150/400, n_estimators=300)
  LGBM: LGBMRegressor(alpha=0.5, num_leaves=20/40, max_depth=10, n_estimators=300/1000)

Feature subset for EnbPI/SPCI:
...
class _LEARSingle:
    ...
    def fit(self, X, y):
        # y can be 2D (n, n_h); use first column
        ...
        # Actually X is already the CP-subset (360 cols), scale uniformly
...
# ── EnbPI and SPCI via SPCI_and_EnbPI class ──────────────────────────────────
...
def _run_spci_class(model_name, data, B, use_spci, label,
    ...
    for si, bs in enumerate(steps):
        ...
        # Run SPCI_and_EnbPI — fit bootstrap ONCE, compute intervals for both alphas.
        # Paper fits two separate objects (identical data, different alpha) which doubles
        # runtime needlessly. We fit once and call compute_PIs_Ensemble_online twice.
        q_map = {0.5: mu_all.astype(np.float32)}

        try:
            X_tr_t = torch.from_numpy(X_tr.astype(np.float64)).float()
            X_te_t = torch.from_numpy(X_te.astype(np.float64)).float()   # all stride rows → n1=stride
            ...
            obj = SPCI.SPCI_and_EnbPI(
                ...
                fit_func=base_model)
            # Fit bootstrap models ONCE (expensive: B × stride fits)
            obj.fit_bootstrap_models_online_multistep(
                B=B, fit_sigmaX=False, stride=stride)

            # Compute intervals for both alpha levels (cheap: ~0.001s each)
        ...
        # Ensure we have all 5 quantiles
        # alpha=0.1 → q_map[0.1]=Q10, q_map[0.9]=Q90
        # alpha=0.3 → q_map[0.3]=Q30, q_map[0.7]=Q70
        # q_map[0.5] = centre
...
def run_scp(model_name, data, quantiles=None, retrain_every=None):
    ...
    for si, bs in enumerate(steps):
        ...
        resid_ca = np.abs(y_ca - mu_ca)   # (n_cal, n_h)
        for qi, q in enumerate(quantiles):
            alpha = 2 * min(q, 1 - q)
