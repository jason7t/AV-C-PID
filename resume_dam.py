# Resume the DAM pipeline from the last completed checkpoint.
#
# Detects which result CSV files already exist in results/ and skips
# completed model-method combinations. Run after an interrupted pipeline
# to continue from where it left off without recomputing finished results.
import os, time, warnings
import numpy as np, pandas as pd
warnings.filterwarnings('ignore')

from config      import (RESULTS_DIR, MODEL_NAMES, METHOD_NAMES,
                          QUANTILES, QUANTILE_LABELS, DAM_RETRAIN_EVERY)
from data_loader import load_dam
...
def _load_csv(model, method, n_test, n_h, cols):
    ...
    return np.repeat(p, DAM_RETRAIN_EVERY, axis=0)[:n_test]
...
def main():
    ...
    def _save(p, model, method):
        save_predictions_csv(p, y_te, cols, DAM_RETRAIN_EVERY,
    ...
    for model in MODEL_NAMES:
        ...
        for method in METHOD_NAMES:
            ...
            all_res[model][method]   = compute_all_metrics(y_te, all_preds[model][method], DAM_RETRAIN_EVERY)
            all_trade[model][method] = run_trading(all_preds[model][method],
                                                    y_te, cols, DAM_RETRAIN_EVERY, MARKET)
