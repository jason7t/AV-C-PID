# Full DAM pipeline from scratch.
#
# Trains all four base models (KNN, LEAR, RF, LGBM) and applies all nine
# PI estimation methods sequentially, then computes evaluation metrics and
# trading results.
...
def main():
    ...
    for model in MODEL_NAMES:
        ...
        for method in METHOD_NAMES:
            ...
            all_trade[model][method] = run_trading(all_preds[model][method],
                                                    y_te, cols, DAM_RETRAIN_EVERY, 'DAM')

    print_aps_table(all_res)
    metrics_to_df(all_res).to_csv(os.path.join(RESULTS_DIR, 'DAM_metrics.csv'))
    ...
    pd.DataFrame(rows).set_index(['Model','Method']).to_csv(
        os.path.join(RESULTS_DIR, 'DAM_trading.csv'))
