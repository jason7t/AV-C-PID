"""
volatility_analysis.py — Investigate whether prediction errors are structurally
larger during high-volatility periods. Builds the empirical motivation for
AV-C-PID: if errors scale with volatility, a volatility-aware method should
outperform static-calibration methods.

USAGE
-----
    # Run on all CSVs in results/ automatically:
    python volatility_analysis.py

    # Or on a single file:
    python volatility_analysis.py results/lear_QR_DAM.csv

    # With custom rolling window:
    python volatility_analysis.py --window 336   # 7-day window at BM half-hourly

WHAT IT DOES
------------
1. Flattens the (n_days, n_horizons) actual-price grid into a continuous timeline.
2. Computes a CENTERED rolling volatility series (std of actual prices) —
   centered because this is a diagnostic tool, not a live forecasting method.
3. Third panel shows SIGNED error (actual - Q50), not absolute error, to reveal
   the direction of bias: positive = under-prediction, negative = over-prediction.
4. Correlates rolling volatility against error series.
5. Buckets into volatility deciles and reports mean error per decile.
6. Saves two plots per file:
     <file>_timeline.png  — 3-panel: prices+forecast / volatility / signed error
     <file>_decile.png    — coverage and violation magnitude by volatility decile
"""
import sys
import os
import glob
...
def load_predictions(path):
    """Load a results CSV and return flattened series."""
    import re
    df = pd.read_csv(path)
...
def process_file(path, window, top_n):
    ...
    # ── Signed error: actual - Q50 (positive = under-prediction) ────────────
    ...
    # ── Top-N largest-error timestamps ───────────────────────────────────────
    ...
    # Panel 3: signed error — positive = under-prediction, negative = over-prediction
